...
import { PRODUCTS_TABLE_PRESETS } from "@/hooks/use-products-table-columns"
import { TableColumn } from "@/components/table-column-settings"

interface ProductsTablePresetsProps {
  columns: TableColumn[]
  onApplyPreset: (presetId: string) => void
  className?: string
}

export function ProductsTablePresets({ 
  columns, 
  onApplyPreset, 
  className 
}: ProductsTablePresetsProps) {
  const { t } = useLanguage()
  ...
  const getCurrentPreset = () => {
    ...
    for (const [presetId, preset] of Object.entries(PRODUCTS_TABLE_PRESETS)) {
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        >
          ...
          {currentPreset && (
            <Badge variant="secondary" className="ml-1 text-xs">
              {PRODUCTS_TABLE_PRESETS[currentPreset as keyof typeof PRODUCTS_TABLE_PRESETS].name}
            </Badge>
          )}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-64 p-2" align="end">
        <div className="space-y-2">
          ...
          {Object.entries(PRODUCTS_TABLE_PRESETS).map(([presetId, preset]) => (
            <Button
              key={presetId}
